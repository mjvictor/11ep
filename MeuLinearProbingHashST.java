...
public class MeuLinearProbingHashST<Key, Value> {
    ...
    */
    ...
    public MeuLinearProbingHashST(int m, double alfaInf, double alfaSup) {
        ...
        for(;iPrimes < PRIMES.length && PRIMES[iPrimes] < m; iPrimes++);
        //StdOut.println("iPrimes = " + iPrimes + "; PRIMES[iPrimes] = " + PRIMES[iPrimes] + "; PRIMES.length = " + PRIMES.length + "; m = " + m);
    ...
    private void resize(int k) {
        ...
        MeuLinearProbingHashST<Key, Value> tmp = new MeuLinearProbingHashST<Key, Value>(PRIMES[k], alfaInf, alfaSup);
        for (int i = 0; i < m; i++) {
            if (keys[i] != null) {
                tmp.put(keys[i], vals[i]);
            }
            //StdOut.println("loop resize");
        }
        keys = tmp.keys;
        vals = tmp.vals;
        m = tmp.m;
    ...
    public void put(Key key, Value val) {
        ...
        //         passar de alfaSup.
        if (key == null) throw new IllegalArgumentException("first argument to put()");
        if (val == null) {
            delete(key);
            return;
        }
        ...
        //StdOut.println("alfa = " + alfa + "; alfaSup = " + alfaSup);
        if (alfa >= alfaSup && iPrimes < PRIMES.length) {
            iPrimes++;        
            resize(iPrimes);
        }
    ...
    public void delete(Key key) {
        ...
        if (((double)n / (double)m) < alfaInf && iPrimes > 0) {
            iPrimes--;
            resize(iPrimes);
        }
    ...
    private boolean check() {
        ...
        if ((double)n/(double)m > alfaSup) {
            System.err.println("Hash table size m = " + "; array size n = " + n);
            return false;       
        }
    ...
    public double averageSearchHit() {
        ...
        for (Key k : keys()) {
            ...
            //nao sei se considera ele mesmo ou nao
            count++;
        }
